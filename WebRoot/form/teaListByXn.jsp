...
<html:html lang="true">
    <head>
        <html:base/><title>按学年显示教师课程列表</title>
...
//		 var facultyString=<%=request.getAttribute("facultyString") %>
        ...
        function f_showCustomers()
        {
            g = $("#maingrid").ligerGrid({
                columns: [
                { display: '学年', name: 'XN', align: 'center'}
                ],   
...
data:xnString,
        ...
        function f_getOrdersData(XN)
        {
            var data = { Rows: [] };
            for (var i = 0; i < allTeaString.Rows.length; i++)
            {
                if (allTeaString.Rows[i].XN == XN)
...
//							{ display: '学院', name: 'DEPT', width: 150,align:'center' },                          
							{ display: '系名', name: 'FACULT', width: 50,align:'center' ,render: function (row)
                                {								
                                    var html = '<a href="../AdminCheckTea.do?method=showByXN&XN=' + row.XN + '&FACULT=' + row.FACULT + '" >' + row.FACULT + '</a>';
                                    return html;							                                
                                }
							},
...
//							{ display: '教师号', name: 'TID', width: 50,align:'center' },  
							{ display: '教师', name: 'TNAME', width: 60,align:'center' ,render: function (row)
                                {								
                                    var html = '<a href="../AdminCheckTea.do?method=showByTnameY&TNAME=' + row.TNAME + '&XN=' + row.XN + '" >' + row.TNAME + '</a>';
                                    return html;							                                
                                }
                            },
							...
							{ display: '课程名称', name: 'CNAME', width: 150,align:'center' ,render: function (row)
                                {								
                                    var html = '<a href="../AdminCheckTea.do?method=showByCnameY&CNAME=' + row.CNAME + '&XN=' + row.XN + '" >'+row.CNAME+'</a>';
                                    return html;							                                
                                }
                            },
							{ display: '课程性质', name: 'COURSETYPE', width: 110,align:'center' ,render: function (row)
                                {								
                                    var html = '<a href="../AdminCheckTea.do?method=showByCourseTypeY&COURSETYPE=' + row.COURSETYPE + '&XN=' + row.XN + '" >'+row.COURSETYPE+'</a>';
                                    return html;							                                
                                }
                            },
...
//	                                    var html = '<p>无评价</p>';
                ...
                data: f_getOrdersData(row.XN) , showTitle: false, columnWidth: 100
